...
(function(root, factory) {
    ...
    canvas_g.width = 500;
    canvas_g.height = 1;  
    if (canvas_g.getContext)   
    {  
        var ctx_g = canvas_g.getContext("2d");         
        var gradient = ctx_g.createLinearGradient(0, 0, 500, 0);
        gradient.addColorStop(0, 'blue');
        gradient.addColorStop(0.033, 'cyan');
        gradient.addColorStop(0.05, '#52D017');
        gradient.addColorStop(0.066, 'yellow');
        gradient.addColorStop(4 / 50, 'orange');
        gradient.addColorStop(5 / 50, 'red');
        gradient.addColorStop(5 / 5, '#C11B17');
        ctx_g.fillStyle = gradient;
        ctx_g.fillRect(0, 0, 500, 1);
        imgData = ctx_g.getImageData(0, 0, canvas_g.width, canvas_g.height);
    }
    ...
    var resemble = function(fileData) {
        ...
        var errorPixelTransform = {
            ...
            flatDifferenceIntensity: function(px, offset, d1, d2) {
                var dis = colorsDistance(d1, d2) / 255//colorsDistance_square(d1,d2);
                var idx = Math.floor(dis * canvas_g.width);
                px[offset] = imgData.data[idx*4];
                px[offset + 1] = imgData.data[idx*4+1];
                px[offset + 2] = imgData.data[idx*4+2];
                px[offset + 3] = imgData.data[idx*4+3];
            },
        ...
        function colorsDistance_square(c1, c2) {
            return (
                Math.sqrt(Math.pow((c1.r - c2.r)/255.0,2) 
                + Math.pow((c1.g - c2.g)/255.0,2) 
                + Math.pow((c1.b - c2.b)/255.0,2))
            );
        }
        ...
        function copyPixel(px, offset, pix) {
			...
			if (errorType === "flatDifferenceIntensity") {
                px[offset] = imgData.data[0];
                px[offset + 1] = imgData.data[1];
                px[offset + 2] = imgData.data[2];
                px[offset + 3] = imgData.data[3];
